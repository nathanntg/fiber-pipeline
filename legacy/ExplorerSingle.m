classdef ExplorerSingle < handle
    ...
    methods
        function EX = ExplorerSingle(video, video_time_between, pixel_size)
            % dimensions
            ...
            EX.video_r = reshape(video, [], EX.d_frames);
            EX.video_tm = cumsum(video_time_between);
            EX.pixel_size = pixel_size;
            
            % calculate standard deviation
            EX.video_s = std(video_smooth2(video, 3, 3), 0, 3);
        ...
        function extractActivity(EX, i, j)
            ...
            % open figure
            figure;
            plot(EX.video_tm, traces);
            xlim(EX.video_tm([1 end]));
        end
        ...
        function updateImage(EX, type, dr) % type: 1 - raw, 2 - std; % dr: [], [min max], [mask]
            ...
            elseif numel(dr) == (EX.d_width * EX.d_height)
                ...
                disp(dr);
                disp([min(im(:)) max(im(:))]);
                im = mat2gray(im, double([min(t) max(t)]));
            else
